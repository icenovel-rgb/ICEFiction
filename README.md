...
- 대사 쓰기 도우미: 따옴표 자동 짝(`"` → `""` + 커서 안쪽, 닫을 때 건너뛰기, 선택 감싸기),
  `Shift+Enter` = 문단 간격 없는 줄바꿈, 보기 옵션 "연속 대사는 붙이기"
- 서식 단축키 `Ctrl+B/I/U` (밑줄은 마크다운에 없어 표준 HTML `<u>`로 기록)
- 줄 앞 `--` → 불릿 `• `, `-` 세 번 → 가로 구분선 `---`
- 문법·단축키 도움말: 상단바 `?` 또는 `F1` (원고에 남는 문법 / 화면 설정을 꼬리표로 구분)
